...
public partial class UserRequestInfo
{
    [Inject]
    ...
    private InitialRequestInfoResponse? document;
    ...
    private async Task<InitialRequestInfoResponse?> GetDocumentAsync(string documentId)
    {
        try
        {
            var documentResponse = await HttpRepository.GetAsync<InitialRequestInfoResponse>($"api/documents/initialRequests/{documentId}");
